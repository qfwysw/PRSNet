...
class MambaBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        self.conv2d = nn.Conv2d(in_channels=config.d_model, out_channels=config.d_model, kernel_size=3, stride=1, padding=1)
        ...
        self.local_relation = nn.Sequential(
            nn.Conv2d(in_channels=config.d_model, out_channels=config.d_model, kernel_size=3, stride=1, padding=1),
            nn.SiLU(),
            nn.Conv2d(in_channels=config.d_model, out_channels=config.d_inner, kernel_size=3, stride=1, padding=1),
        )
        ...
        # torch.arange(1, 16 + 1, dtype=torch.float32).repeat(4, 1)
    ...
    def forward(self, x):
        ...
        h = 1
        # xyz = rearrange(x, "b (h w) d -> b d h w", h=h)
        # xyz = rearrange(local_relation, "b d h w -> b d (h w)")
        ...
        xyz = F.silu(xyz)
        xyz = self.conv2d(xyz)
        xyz = rearrange(xyz, "b d h w -> b d (h w)")
        xyz = xyz.transpose(1, 2)
        # local_relation = self.local_relation(rearrange(x, "b (h w) d -> b d h w", h=h))
        # local_relation = rearrange(local_relation, "b d h w -> b d (h w)")
        # import pdb;pdb.set_trace()
        # 修改部分↑
